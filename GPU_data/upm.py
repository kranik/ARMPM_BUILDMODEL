...
#testset = ["binomialOPtions","BlackScholes","MonteCarloMultiGPU","particles","SobolQRNG","convolutionTexture","nbody","radixSortThrust","transpose"]
frequency_ref_h = "998"
voltage_ref_h = "1.07"
#static_ref = "0.3336"
static_ref_h = "0.31"
...
r_file.write("time step,temperature,measured,unified model,unified model temp,per-frequency model,frequency,static,static percentage\r\n")
...
for x in range(13):
	...
	#path0 = "./files_13pc_3_rodinia/power_measurement_merged_global.txt"
	#path0 = "./run_0_nvidia3/power_measurement_merged_global.txt" #pause with fan 255 before run
	#path0 = "./run_255_nvidia/power_measurement_merged_global.txt"
	#path0 = "./run_128_nvidia/power_measurement_merged_global.txt"
	#path0 = "./run_32_nvidia/power_measurement_merged_global.txt"
	path0 = "./run_64_nvidia/power_measurement_merged_global.txt"
	...
	for l_0 in c_0: #read lines one by one
		...
		if benchmark in testset:
			if frequency == frequency_cur[x]:
				...
				#predicted_static_l_t = float(w_0[4])*0.0114-0.1243
				#predicted_static_l_t = float(w_0[4])*0.0066+0.0425
				#predicted_static_l_t = float(w_0[4])*0.0071+0.0273
				#predicted_static_l_t = (float(w_0[4]))*0.0082+0.0273
				predicted_static_l_t = (float(w_0[4])+0.0)*0.0051+0.0849
				#predicted_static_l_t = (float(30.0))*0.0055+0.0754
				#good predicted_static_l_t = (float(w_0[4])+5.0)*0.0055+0.0754
				#predicted_static_l_t = (float(w_0[4]))*0.0051+0.0849
				#predicted_static_l_t = 0.0011*(float(w_0[4]))*(float(w_0[4]))-0.0546*(float(w_0[4]))+0.8637


				#predicted_static_l_t_255 = (float(temp_255[x]))*0.0072+0.0173
				#predicted_static_l_t_255 = 0.25 #for low temperature
				#predicted_static_l_t_255 = 0.15  #for high temperature
				#predicted_static_l_t_255 = (float(w_0[4])-1.0)*0.0051+0.0849 # low temperature 255
				#predicted_static_l_t_255 = (float(w_0[4])-4.0)*0.0051+0.0849 # mid temperature 128

				#predicted_static_l_t_255 = (float(w_0[4])-36.0)*0.0051+0.0849 #high tempeature 0
				#predicted_static_l_t_255 = (float(w_0[4])-27.0)*0.0051+0.0849 #high tempeature 32
				print("Reference temperature ",float(w_0[4])-4.0)
				# temperature correction of -1.0 at fan 255 because it matches training point (chip vs sensor difference ?)
				# temperature correction of -36.0 at fan 0 to estimate static power if run with fan at 255 and freq 384/0.82v
				#choose one of the ones below depending on fan speed.
				#predicted_static_l_t_255 = 15.0*0.0051+0.0849 # fan 0
				#predicted_static_l_t_255 = 30.0*0.0051+0.0849 # fan 255
				#predicted_static_l_t_255 = 18.0*0.0051+0.0849 # fan 32
				#predicted_static_l_t_255 = 29.0*0.0051+0.0849 # fan 128
				predicted_static_l_t_255 = 22.0*0.0051+0.0849 # fan 64
				...
				predicted_dynamic_l_t_v = predicted_power_u_l_t - predicted_static_l_t_v
				...
				temperature = float(w_0[4])
				average_temperature = average_temperature + float(w_0[4])
				static_percentage = (predicted_static_l_t_v*100)/(predicted_dynamic_l_t_v+predicted_static_l_t_v)
				#print("predicted_u_l ",predicted_power_u_l,"predicted_u_l_t ",predicted_power_u_l_t,"measured ",measured_power)
				r_file.write("%d,%f,%f,%f,%f,%f,%d,%f,%f\r\n" % (line_count,temperature,measured_power,predicted_power_u_l,predicted_power_u_l_t,predicted_power_f,int(frequency),predicted_static_l_t_v,static_percentage))
