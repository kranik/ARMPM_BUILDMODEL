...
The benchmark split file needs to have a specific header and two columns for the train and test set with the benchmark names distributed between the two categories. More information about what heuristics I use to identify the best set of events and the model optimisation criteria are explained in details in my [_dissertation_](https://seis.bristol.ac.uk/~eejlny/downloads/kris_thesis.pdf). 
