...
def main(argv):
    ...
    for opt, arg in opts:
        if opt == '-h':
            print 'Usage: -p <plot type> -x <x axis label> -y <y axis label> -b <physical measurements benchmark file> -l <benchmark label> -i <model input file> -a <mode label> -o <output file>'
            sys.exit(2)
        elif opt in ("-p", "--ptype"):
            if plottype != 0:
    ...
    if plottype == 1:   
        ...
        ax.plot(sample_np-min(sample),phys_value,label=blabel,color = c)
        
        for num, infile in enumerate(inputfile):
            c=next(color)
            ax.plot(sample_np-min(sample)+1,pred_value[num],label=inputlabel[num],color = c)
        
        ax.set_xlim(1,len(sample))
        sample_np = np.insert((sample_np[499::500]-min(sample)+1),0,1)
        ax.set_xticks(sample_np)
        ax.set_xticklabels(sample_np,fontsize='small',rotation='45')
